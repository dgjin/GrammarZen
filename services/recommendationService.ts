...
export const getRecommendations = async (
  ...
  try {
    ...
    if (!response.ok) {
      // 如果后端API不可用，使用本地备用逻辑
      return getLocalRecommendations(text, history);
    }
 ...
 * 分析文本并获取推荐
 */
export const analyzeText = async (text: string): Promise<Recommendation[]> => {
  try {
    const response = await fetch(`${API_BASE_URL}/recommendations/analyze`, {
      method: 'POST',
      headers: {
    ...
    // 如果请求失败，使用本地备用逻辑
    return getLocalRecommendations(text);
  }
};
